...
Before opening collection, complete and review /privacy, /marketing and /terms, verify cross-border processing details and legal basis, choose a retention/deletion workflow, implement production-grade abuse/rate limiting, and test withdrawal procedures. Current basic validation/origin checks/honeypot are not a complete anti-abuse system. Existing public INSERT policy is outside this application and remains unchanged.
...
Supabase target: existing `public.launch_signups`. The API writes only existing fields and preserves INSERT-only RLS. No migrations or deletion were performed. Default metadata prevents indexing while launch preparation is incomplete.
