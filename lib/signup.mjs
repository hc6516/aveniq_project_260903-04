import { captureAttribution } from './utm.mjs';
...
export function validateSignup(body) {
  ...
  if (typeof body.phone !== 'string' || (body.email != null && typeof body.email !== 'string')) return { error: '휴대폰 번호와 이메일 형식을 확인해 주세요.' };
  const phone = body.phone.replace(/[\s-]/g, '');
  const email = body.email?.trim().toLowerCase() || null;
  if (!/^010\d{8}$/.test(phone)) return { error: '010으로 시작하는 휴대폰 번호를 입력해 주세요.' };
  if (email && (email.length > 254 || !/^[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+$/.test(email))) return { error: '올바른 이메일 주소를 입력해 주세요.' };
  if (body.emailMarketing === true && !email) return { error: '이메일 수신 동의를 선택한 경우 이메일 주소를 입력해 주세요.' };
  ...
  return { data: { phone, email, ...captureAttribution('', body.attribution), privacy_agreed_at: now, privacy_consent_version: CONSENT_VERSION, marketing_consent: channels.length > 0, marketing_consent_at: channels.length ? now : null, marketing_consent_channels: channels, marketing_consent_version: channels.length ? CONSENT_VERSION : null, source: 'aveniq-web-v2' } };
}
...
export async function handleSignup(request, env = process.env, fetcher = fetch) {
  ...
  try {
    ...
    const headers = { apikey: env.SUPABASE_SECRET_KEY, ...(env.SUPABASE_SECRET_KEY.startsWith('eyJ') ? { Authorization: 'Bearer ' + env.SUPABASE_SECRET_KEY } : {}), 'Content-Type': 'application/json', Prefer: 'return=minimal' };
    // Dedicated server-only AVENIQ table; the legacy shared table remains untouched.
    const response = await fetcher(new URL('/rest/v1/aveniq_signups', supabaseUrl), { method: 'POST', headers, body: JSON.stringify(result.data), signal: AbortSignal.timeout(8000), cache: 'no-store' });
